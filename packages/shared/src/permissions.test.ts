...
describe("hasPermission", () => {
  ...
  it("matches a single required permission", () => {
    expect(hasPermission(["control.start", "file.read"], "file.read")).toBe(true);
    expect(hasPermission(["control.start"], "file.read")).toBe(false);
  });

  it("matches any of several required permissions", () => {
    expect(hasPermission(["control.stop"], ["control.start", "control.stop"])).toBe(true);
    expect(hasPermission(["file.read"], ["control.start", "control.stop"])).toBe(false);
  });

  it("treats * as full access", () => {
    expect(hasPermission([ALL_PERMISSIONS_WILDCARD], "backup.restore")).toBe(true);
  });
});
...
describe("license feature helpers", () => {
  ...
  it("normalizes feature lists (null = unrestricted)", () => {
    ...
    expect(normalizeLicenseFeatures(["power", "power", "nope"])).toEqual(["power"]);
  ...
  it("applies a license ceiling to granted permissions", () => {
    expect(applyLicenseFeatureCeiling(["control.start", "file.read"], null)).toEqual([
      "control.start",
      "file.read",
    ]);
    expect(applyLicenseFeatureCeiling(["control.start", "file.read"], ["power"])).toEqual([
      "control.start",
    ]);
