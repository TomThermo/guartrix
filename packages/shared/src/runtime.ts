...
export function consoleLineIndicatesBootFailure(line: string, type?: ServerType): boolean {
...
export function dockerImageForServerType(type: ServerType, javaVersion?: string | null): string {
  switch (type) {
    case "BEDROCK":
    case "BEDROCK_PREVIEW":
...
export function runtimeLabelForServerType(type: ServerType, javaVersion?: string | null): string {
  switch (runtimeKindFor(type)) {
    case "bedrock_native":
      return "Bedrock BDS";
    case "php":
