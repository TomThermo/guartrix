...
import { MYSQL_CONTAINER, withMysqlDefaultsFile } from "./mysql-network.js";
import { ensureMysql, getMysqlStatus, mysqlPort, mysqlPublicHost } from "./mysql-container.js";
...
function assertIdent(value: string, label: string): string {
  if (!/^[a-zA-Z0-9_]{2,64}$/.test(value)) {
    throw new Error(`Invalid MySQL ${label}: use 2–64 letters, digits or underscore`);
  }
  return value;
}
...
export function assertSafeMysqlRemote(
  ...
  if (!privateOk) {
    throw new Error("MySQL remote must be a private Docker/LAN pattern (e.g. 172.%)");
  }
  return value;
}
...
async function mysqlExecSql(sql: string): Promise<string> {
  return withMysqlDefaultsFile(async (defaultsFile) => {
    const { stdout, stderr } = await docker(
      ["exec", MYSQL_CONTAINER, "mysql", `--defaults-extra-file=${defaultsFile}`, "-N", "-e", sql],
      { timeout: 30_000 },
    );
...
export async function rotateMysqlPassword(input: {
  ...
  await mysqlExecSql(
    [`ALTER USER '${user}'@'${host}' IDENTIFIED BY '${pass}';`, `FLUSH PRIVILEGES;`].join(" "),
  );
...
export async function dumpMysqlDatabaseToFile(name: string, destPath: string): Promise<void> {
  await ensureMysql();
...
export async function restoreMysqlDatabaseFromFile(name: string, sqlPath: string): Promise<void> {
  await ensureMysql();
  const db = assertIdent(name, "database name");
  ...
  await withMysqlDefaultsFile(async (defaultsFile) => {
    ...
    await new Promise<void>((resolve, reject) => {
      const child = spawn(
        "docker",
        ["exec", "-i", MYSQL_CONTAINER, "mysql", `--defaults-extra-file=${defaultsFile}`, db],
