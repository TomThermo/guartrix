...
export function mysqlRootPassword(): string {
  ...
  throw new Error("MYSQL_ROOT_PASSWORD is not set — restart the panel/daemon to bootstrap it");
}
...
export async function connectContainerToSharedNetwork(containerName: string): Promise<void> {
  await ensureGuartrixNetwork();
  try {
