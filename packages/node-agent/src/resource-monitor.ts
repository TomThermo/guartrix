/**
 * Wings-style background resource polling per running Minecraft container.
...
function toServerStats(
  engine: EngineStats,
  startedAtMs: number | null,
): ServerStats {
  const cores = hostCpuCount();
  // UI expects ~0–100 of host; Wings absolute can be 600% on 6 cores.
...
class ResourceMonitor extends EventEmitter {
  ...
  private publish(serverId: string, stats: ServerStats): void {
    ...
    // Wings CachedUsage style — never block the stats stream on a dir walk
  ...
  private async run(serverId: string, poll: ActivePoll): Promise<void> {
    ...
    try {
      ...
      poll.startedAtMs = await containerStartedAtMs(name, signal).catch(
        () => null,
      );
