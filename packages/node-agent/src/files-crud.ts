...
export async function createDirectory(serverId: string, relPath: string): Promise<void> {
  await fixDataOwnership(serverDir(serverId));
  const { absolute, relative } = resolveSafePath(serverId, relPath);
...
export async function deletePath(serverId: string, relPath: string): Promise<void> {
  await fixDataOwnership(serverDir(serverId));
  const { absolute, relative } = resolveSafePath(serverId, relPath);
...
export async function renamePath(serverId: string, fromRel: string, toRel: string): Promise<void> {
  await fixDataOwnership(serverDir(serverId));
...
export async function saveUpload(
  ...
  // Fail fast when already at/over quota
