...
export const firewallEnabled = (process.env.MANAGE_FIREWALL ?? "true").toLowerCase() !== "false";
...
export async function changeFirewallPort(oldPort: number, newPort: number): Promise<void> {
  if (!firewallEnabled) return;
