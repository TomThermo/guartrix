...
export function extraVolumeArgs(
  mounts: DaemonServerConfig["extraMounts"],
): string[] {
...
export async function computeDiskUsageMessage(
  serverId: string,
  dir: string,
): Promise<string> {
  try {
    ...
    const limit =
      (await filesystemSizeBytes(dir)) ?? 80 * 1024 * 1024 * 1024;
