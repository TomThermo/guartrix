/**
 * Disk quota helpers (Wings-style): check before writes; stop when over while running.
 */
...
export async function writeServerLimits(
  serverId: string,
  limits: ServerLimits,
): Promise<void> {
...
export async function assertDiskSpace(
  ...
  let used = peekDiskUsage(serverId)
    ? Math.max(
        0,
        (peekDiskUsage(serverId)!.totalBytes -
          peekDiskUsage(serverId)!.backupsBytes),
      )
