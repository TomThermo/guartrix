...
export function schedulePersistHistory(
  host: ConsoleHost,
  serverId: string,
): void {
...
export function trackPlayerLine(
  host: ConsoleHost,
  serverId: string,
  line: string,
): void {
...
export function daemonSay(
  ...
  pushConsoleLine(
    host,
    serverId,
    `[${formatDaemonClock()}] [Guartrix Daemon] ${message}`,
  );
}

export async function emitDiskUsage(
  host: ConsoleHost,
  serverId: string,
): Promise<void> {
...
export async function emitStartupBanner(
  ...
  daemonSay(
    host,
    serverId,
    `Resuming log starting from: ${formatDaemonStamp()}`,
  );
