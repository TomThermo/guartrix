...
const DOCKER_SOCKET =
  process.env.DOCKER_SOCK?.trim() || process.env.DOCKER_SOCKET?.trim() || "/var/run/docker.sock";
...
/** Same formula as Docker CLI (inactive_file subtracted). */
export function calculateDockerMemory(stats: DockerMemoryStats): number {
  const usage = stats.usage ?? 0;
  const map = stats.stats ?? {};
  if (typeof map.total_inactive_file === "number" && map.total_inactive_file < usage) {
...
/**
 * Absolute CPU% across all cores (can exceed 100), matching Docker /
 * `docker stats` CPUPerc semantics before host normalisation.
 */
export function calculateDockerAbsoluteCpu(precpu: DockerCpuStats, cpu: DockerCpuStats): number {
...
export async function fetchContainerStatsOnce(
  ...
  const res = await dockerRequest("GET", `/containers/${enc}/stats?stream=0`, {
    signal,
    timeoutMs: 12_000,
  });
...
/**
 * Stream container stats until aborted.
...
export function dockerHostHint(): string {
  try {
    return new URL(process.env.DOCKER_HOST ?? "unix:///var/run/docker.sock").pathname;
  } catch {
    return DOCKER_SOCKET;
  }
}
