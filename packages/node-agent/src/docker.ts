...
export async function cleanupLeftoverContainers(): Promise<number> {
  ...
  for (const label of ["guartrix.server", "blockhost=1"]) {
    const filter =
      label === "guartrix.server" ? "label=guartrix.server" : `label=${label}`;
...
export async function getStatsByName(name: string): Promise<DockerStatsRaw | null> {
  try {
    const { stdout } = await docker(
      ["stats", name, "--no-stream", "--format", "{{json .}}"],
      { timeout: 10_000 },
    );
...
export async function getStatsForContainers(
  names: string[],
): Promise<Map<string, DockerStatsRaw>> {
  const result = new Map<string, DockerStatsRaw>();
  if (!names.length) return result;
  try {
    const { stdout } = await docker(
      ["stats", ...names, "--no-stream", "--format", "{{json .}}"],
      { timeout: 15_000 },
    );
