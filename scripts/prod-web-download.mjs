...
const DOWNLOAD_SIGNING_SECRET =
  process.env.DOWNLOAD_COOKIE_SECRET?.trim() || process.env.SESSION_SECRET?.trim() || "";
...
export function logDownloadStatus(rootDir) {
  ...
  console.log(`[guartrix] Password downloads → /download (dir ${downloadDir(rootDir)})`);
}
...
function downloadSetCookieHeader(token, httpsEnabled) {
  const secure =
    process.env.SESSION_SECURE === "true" || process.env.SESSION_SECURE === "1" || httpsEnabled;
  const parts = [
