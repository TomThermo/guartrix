...
export function loadProdWebConfig(rootDir) {
  ...
  const DOWNLOAD_PUBLIC_HOST = (process.env.DOWNLOAD_PUBLIC_HOST?.trim() || "").toLowerCase();
  ...
  const DEFAULT_LICENSE_LE_CERT = "/etc/letsencrypt/live/license.guartrix.com/fullchain.pem";
  const DEFAULT_LICENSE_LE_KEY = "/etc/letsencrypt/live/license.guartrix.com/privkey.pem";
