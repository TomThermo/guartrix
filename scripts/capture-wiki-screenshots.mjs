 ...
 *   GUARTRIX_PALETTE=crimson           — force Appearance palette (default Crimson Graphite)
 *   GUARTRIX_THEME=dark                — force theme dark|light|system (default dark)
 *
...
/** Default wiki look: Crimson Graphite (`crimson`) + dark. */
const PALETTE = (process.env.GUARTRIX_PALETTE || "crimson").trim() || "crimson";
const THEME = (process.env.GUARTRIX_THEME || "dark").trim() || "dark";
const VIEWPORT = (() => {
...
const TOTP_FROM_DB =
  process.env.GUARTRIX_TOTP_FROM_DB === "1" || process.env.GUARTRIX_TOTP_FROM_DB === "true";
...
const puppeteerPath = process.env.PUPPETEER_MODULE || "/tmp/gx-shot/node_modules/puppeteer";
  ...
  await page.evaluate(
    ({ demoName, scrubIps, hide2fa }) => {
      ...
      for (const row of document.querySelectorAll(".server-row")) {
        const nameEl = row.querySelector(".server-row__name, .server-row-name");
        const name = (nameEl?.textContent || row.textContent || "").trim();
        if (!new RegExp(`\\b${demoName.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}\\b`, "i").test(name)) {
          row.remove();
        }
      }
      ...
      const ipRe = /\b(?!127\.0\.0\.1)(?:\d{1,3}\.){3}\d{1,3}\b/g;
      const walk = (node) => {
      ...
      for (const el of document.querySelectorAll(
        ".font-monospace, code, .join-card, .server-row-meta",
      )) {
        if (!el.childElementCount && el.textContent) {
          el.textContent = el.textContent.replace(/\b\d{1,3}(?:\.\d{1,3}){3}\b/g, (m) =>
...
async function clickTab(page, label) {
  const clicked = await page.evaluate((text) => {
    ...
    const el = nodes.find((n) => (n.textContent || "").replace(/\s+/g, " ").trim() === text);
    if (!el) return false;
    el.click();
    return true;
  }, label);
...
async function dismissModals(page) {
  ...
  await page
    .evaluate(() => {
      document
        .querySelectorAll(".modal.show .btn-close, .modal.show [data-bs-dismiss='modal']")
        .forEach((b) => b.click());
    })
    .catch(() => {});
...
async function main() {
  ...
  // Force Crimson Graphite (and theme) before any page paint — Appearance localStorage.
  await page.evaluateOnNewDocument(
    ({ palette, theme }) => {
      try {
        localStorage.setItem("guartrix.palette", palette);
        localStorage.setItem("guartrix.theme", theme);
      } catch {
        // ignore
      }
    },
  ...
  console.log(
    `Capturing ${BASE} as ${USER} @ ${VIEWPORT.width}x${VIEWPORT.height} palette=${PALETTE} theme=${THEME}`,
  );
  ...
  if (PLACEHOLDER_PLAYERS) {
    ...
    page.on("request", (req) => {
      const url = req.url();
      try {
        if (req.method() === "GET" && /\/api\/servers\/[^/]+\/online(?:\?|$)/.test(url)) {
          const body = {
            online: true,
  ...
  await Promise.all([
    page
      .waitForFunction(
        () =>
          Boolean(document.querySelector("#totp-code")) || !location.pathname.includes("/login"),
        { timeout: 60000 },
      )
      .catch(() => undefined),
  ...
  // Dashboard — wait for the demo server row (list is async)
  await page.goto(`${BASE}/`, { waitUntil: "networkidle2" });
  await page
    .waitForFunction(
      (demoName) =>
        [...document.querySelectorAll(".server-row__name, .server-row-name")].some((el) =>
          new RegExp(`\\b${demoName}\\b`, "i").test((el.textContent || "").trim()),
        ),
      { timeout: 20000 },
      DEMO_NAME,
    )
    .catch(() => console.warn("dashboard: demo server row not found in time"));
  await shot(page, "02-dashboard.png", { delay: 800 });
  ...
  // Account (Security tab — Profile is default now)
  await page.goto(`${BASE}/account/security?tab=security`, { waitUntil: "networkidle2" });
  await shot(page, "27-account-security.png");

  await page.goto(`${BASE}/account/security?tab=appearance`, { waitUntil: "networkidle2" });
  // Keep Appearance open briefly so palette UI matches Crimson Graphite shots elsewhere
  ...
  // Import archive tab
  await page.goto(`${BASE}/servers/new`, { waitUntil: "networkidle2" });
  if ((await clickText(page, "Import archive")) || (await clickText(page, "Import"))) {
    await new Promise((r) => setTimeout(r, 700));
