...
try {
  ...
  try {
    ...
    // Screenshot capture cannot pass Cloudflare Turnstile — disable for clean docs shots.
    // Re-enable afterwards under Admin → Security (or restore from backup).
    raw.turnstileEnabled = false;
    fs.writeFileSync(settingsPath, `${JSON.stringify(raw, null, 2)}\n`);
    console.log("cleared twoFactorRequiredRoles + disabled turnstile in panel-settings.json");
    console.log("(restart panel / API so settings apply before capturing)");
  } catch (err) {
