...
function tagFor(route) {
  ...
  if (
    ...
    route.startsWith("/api/account") ||
    route.startsWith("/api/invites")
  )
    return "Account";
...
function securityFor(route, tag) {
  if (
    route.startsWith("/api/auth/register") ||
    route.startsWith("/api/auth/forgot-password") ||
    route.startsWith("/api/auth/reset-password") ||
    route.startsWith("/api/auth/verify-email")
  ) {
    return null;
  }
...
const routeRe2 = /\bapp\.(get|post|put|patch|delete)\s*<[\s\S]*?>\s*\(\s*["'`](\/[^"'`]+)["'`]/g;
...
function stubPath(route) {
  ...
  for (const method of methods) {
    ...
    if (sec)
      lines.push(
        `      security: ${JSON.stringify(sec).replace(/"/g, "")}`.replace(/(\w+):/g, "$1:"),
      );
