...
export async function getRedisStatus(): Promise<RedisStatus> {
  ...
  const rateLimitStore = (process.env.RATE_LIMIT_STORE || "file").trim().toLowerCase();
...
export async function acquireSchedulerLock(): Promise<boolean> {
  ...
  try {
    ...
    const result = await redis.set(SCHEDULER_LOCK_KEY, INSTANCE_ID, "PX", ttl, "NX");
    return result === "OK";
  } catch (err) {
    lastError = err instanceof Error ? err.message : String(err);
...
export async function acquireBridgeLock(): Promise<boolean> {
  ...
  try {
    ...
    const result = await redis.set(BRIDGE_LOCK_KEY, INSTANCE_ID, "PX", ttl, "NX");
    return result === "OK";
  } catch (err) {
    lastError = err instanceof Error ? err.message : String(err);
    return false;
...
export function onPanelBusEvent(listener: (event: PanelBusEvent) => void): () => void {
