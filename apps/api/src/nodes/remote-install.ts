 ...
 */
export function runRemoteDaemonInstall(input: RemoteInstallInput): Promise<RemoteInstallResult> {
  ...
  return new Promise((resolve) => {
    ...
    const finish = (result: RemoteInstallResult) => {
      ...
      resolve({
        ...
        trustedHostKeyFingerprint: result.trustedHostKeyFingerprint ?? trustedFp,
      });
    };
      ...
      conn.exec(remoteCmd, { pty: true }, (err, stream) => {
        ...
        stream.on("close", (code: number | null) => {
          ...
          finish({
            ...
            stderr,
            error:
              exitCode === 0 ? undefined : `Remote install exited with code ${exitCode ?? "?"}`,
          });
        });
      });
    });
...
export function defaultRepoUrl(): string {
  return process.env.GUARTRIX_REPO_URL?.trim() || "https://github.com/TomThermo/guartrix.git";
}

export function panelPublicBase(): string {
  return (process.env.PUBLIC_BASE_URL?.trim() || `https://${config.publicHost}`).replace(/\/$/, "");
}
