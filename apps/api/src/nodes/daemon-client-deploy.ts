...
export async function daemonPeerDeployArchiveOnNode(
  ...
  if (!data.ok) {
    ...
    throw new DaemonHttpError(text || `Peer deploy failed (${data.status})`, data.status);
  }
...
export async function daemonDeployArchiveFileOnNode(
  ...
  form.append("file", await openAsBlob(archivePath), `${serverId}.tar.gz`);
...
export async function daemonDeployFromDir(serverId: string, localDir: string): Promise<void> {
  ...
  const tmp = path.join(os.tmpdir(), `guartrix-push-${serverId}-${Date.now()}.tar.gz`);
  try {
    ...
    const form = new FormData();
    form.append("file", new Blob([buf], { type: "application/gzip" }), `${serverId}.tar.gz`);
    const res = await daemonFetch(node, `/servers/${serverId}/deploy`, {
      method: "POST",
      body: form,
      timeoutMs: DAEMON_LONG_TIMEOUT_MS,
...
export async function daemonExportArchiveToFile(serverId: string, destPath: string): Promise<void> {
  const { node } = await resolveNodeForServer(serverId);
...
export async function daemonExportArchive(serverId: string): Promise<Buffer> {
  ...
  const os = await import("node:os");
  const path = await import("node:path");
  const tmp = path.join(os.tmpdir(), `guartrix-export-${serverId}-${Date.now()}.tar.gz`);
  try {
...
export async function daemonWipeServerOnNode(serverId: string, nodeId: string): Promise<void> {
  const node = await resolveNode(nodeId);
