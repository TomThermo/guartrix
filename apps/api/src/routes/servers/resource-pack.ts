...
export function registerResourcePackRoutes(app: FastifyInstance): void {
  ...
  app.get<{ Params: { token: string } }>("/api/public/packs/:token.zip", async (request, reply) => {
  ...
  app.get<{ Params: { id: string } }>("/api/servers/:id/resource-pack", async (request, reply) => {
  ...
  app.post<{ Params: { id: string } }>("/api/servers/:id/resource-pack", async (request, reply) => {
    const access = await requireServerAccess(request, reply, request.params.id, {
