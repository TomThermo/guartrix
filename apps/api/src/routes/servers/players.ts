...
export function registerServerPlayerRoutes(app: FastifyInstance): void {
  ...
  app.post<{ Params: { id: string }; Body: { name?: string } }>(
    ...
    async (request, reply) => {
      ...
      try {
        const result = await addWhitelist(access.server.id, serverDir(access.server.id), name);
        logActivity({
  ...
  app.delete<{ Params: { id: string; name: string } }>(
    ...
    async (request, reply) => {
      ...
      try {
        ...
        const result = await removeWhitelist(access.server.id, serverDir(access.server.id), player);
        logActivity({
  ...
  app.post<{ Params: { id: string }; Body: { name?: string; level?: number } }>(
    ...
    async (request, reply) => {
      ...
      try {
        ...
        const result = await addOp(access.server.id, serverDir(access.server.id), name, level);
        logActivity({
          action: "player.op",
          request,
          user: access.user,
  ...
  app.delete<{ Params: { id: string; name: string } }>(
    ...
    async (request, reply) => {
      ...
      try {
        const player = decodeURIComponent(request.params.name);
        const result = await removeOp(access.server.id, serverDir(access.server.id), player);
        logActivity({
          action: "player.deop",
          request,
          user: access.user,
  ...
  app.delete<{ Params: { id: string; name: string } }>(
    ...
    async (request, reply) => {
      ...
      try {
        await fixDataOwnership(serverDir(access.server.id));
        const player = decodeURIComponent(request.params.name);
        const result = await removePlayerBan(access.server.id, serverDir(access.server.id), player);
        logActivity({
          action: "player.unban",
          request,
          user: access.user,
  ...
  app.delete<{ Params: { id: string; ip: string } }>(
    ...
    async (request, reply) => {
      ...
      try {
        ...
        const result = await removeIpBan(access.server.id, serverDir(access.server.id), ip);
        logActivity({
          action: "player.unban",
          request,
          user: access.user,
