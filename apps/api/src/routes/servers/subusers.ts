...
import { z } from "zod";
import { SERVER_PERMISSIONS, type CreateSubUserResponse, type ServerSubUser } from "@msm/shared";
  ...
  app.post<{ Params: { id: string } }>("/api/servers/:id/subusers", async (request, reply) => {
    ...
    if (access.server.ownerId) {
      ...
      if (owner?.email?.toLowerCase() === email || owner?.username.toLowerCase() === email) {
        return reply.status(400).send({ error: "Cannot invite the server owner as a subuser" });
      }
    }
    ...
    if (existing) {
      return reply.status(409).send({ error: "A subuser with this email already exists" });
    }
