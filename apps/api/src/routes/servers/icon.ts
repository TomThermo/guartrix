...
export function registerIconRoutes(app: FastifyInstance): void {
  ...
  app.get<{ Params: { id: string } }>("/api/servers/:id/icon/meta", async (request, reply) => {
    const access = await requireServerAccess(request, reply, request.params.id, {
      permission: "settings.read",
    });
    if (!access) return;
  ...
  app.post<{ Params: { id: string } }>("/api/servers/:id/icon", async (request, reply) => {
    const access = await requireServerAccess(request, reply, request.params.id, {
  ...
  app.delete<{ Params: { id: string } }>("/api/servers/:id/icon", async (request, reply) => {
    const access = await requireServerAccess(request, reply, request.params.id, {
      permission: "settings.update",
    });
    if (!access) return;
