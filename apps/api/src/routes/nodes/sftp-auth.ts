...
async function checkRateLimit(key: string): Promise<boolean> {
  const result = await getRateLimitStore().hit(key, SFTP_AUTH_RATE_WINDOW_MS, SFTP_AUTH_RATE_MAX);
...
async function resolveDaemonNodeFromBearer(token: string) {
  if (looksLikeJwt(token)) {
    ...
    try {
      ...
      const json = Buffer.from(peek.replace(/-/g, "+").replace(/_/g, "/") + pad, "base64").toString(
        "utf8",
      );
...
export function registerSftpAuthRoutes(app: FastifyInstance): void {
  app.post("/api/internal/sftp-auth", async (request, reply) => {
    ...
    const panelOk = verifyPassword(password, user ? user.passwordHash : TIMING_DUMMY_HASH);
    const appOk = user && !panelOk ? await verifyUserAppPassword(user.id, password) : false;
    if (!user || (!panelOk && !appOk)) {
      return reply.status(401).send({ ok: false, error: "Unauthorized" });
    }
