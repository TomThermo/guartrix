...
function safeBillingRedirectUrl(
  ...
  let base: URL;
  try {
    base = new URL(panelBase());
  } catch {
    return fallback;
  }
  if (url.origin !== base.origin) return fallback;
  if (url.username || url.password) return fallback;
  // Stay on the billing account area (query/hash may include payment id).
...
export function registerBillingRoutes(app: FastifyInstance): void {
  ...
  app.post("/api/billing/checkout", async (request, reply) => {
    ...
    const redirectUrl = safeBillingRedirectUrl(
