...
function readPidFile(name: string): number | null {
  try {
    const raw = fs.readFileSync(path.join(config.dataDir, "run", `${name}.pid`), "utf8");
...
function mergeNodeContainers(
  ...
  const seen = new Set(
    dockerContainers.map((c) => c.serverId).filter((id): id is string => Boolean(id)),
  );
  const missing = nodeServers.filter((s) => !seen.has(s.id)).map(stoppedServerRow);
...
export function registerStatusRoutes(app: FastifyInstance): void {
  ...
  app.get("/api/admin/status", async (request, reply) => {
    ...
    const bypass = q.refresh === "1" || q.refresh === "true" || q.nocache === "1";
    ...
    const httpsEnabled = process.env.HTTPS_ENABLED !== "false" && process.env.HTTPS_ENABLED !== "0";
