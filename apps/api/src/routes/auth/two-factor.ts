...
export function registerTwoFactorGuard(app: FastifyInstance): void {
  app.addHook("preHandler", async (request, reply) => {
    ...
    // Only skip the cookie enrollment gate when a real gt_/gta_ key resolved —
    // junk Bearer must not bypass 2FA for session admins (same pattern as CSRF).
    // Key auth still requires enrollment via resolveApiKeyAuth + key mint.
...
export function registerTwoFactorRoutes(app: FastifyInstance): void {
  ...
  app.post<{ Body: { code?: string } }>("/api/auth/2fa/enable", async (request, reply) => {
    const originErr = assertSameOrigin(request);
    if (originErr) return reply.status(403).send({ error: originErr });
    const user = await requireAuth(request, reply);
    if (!user) return;
  ...
  app.post<{ Body: { password?: string; code?: string } }>(
    ...
    async (request, reply) => {
      ...
      if (!(await verifyAccountPassword(request, parsed.data.password))) {
