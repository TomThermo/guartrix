...
export async function enforceUnlicensedFreeTier(reason: string): Promise<number> {
  ...
  const keepIds = new Set(servers.slice(0, UNLICENSED_MAX_SERVERS).map((s) => s.id));
  let n = 0;
  for (const s of servers) {
    ...
    try {
      ...
      n += 1;
      console.warn(`[license] Stopped ${s.name} (${s.id}): unlicensed free tier (${reason})`);
    } catch (err) {
      console.warn(
  ...
  if (n > 0) {
    console.warn(`[license] Stopped ${n} game server(s) for unlicensed free tier: ${reason}`);
  }
  return n;
}

/** Stop running servers that exceed per-server or total license RAM caps. */
export async function stopServersExceedingLicenseQuota(state: LicenseState): Promise<number> {
...
export async function startServerIfLicensed(serverId: string): Promise<void> {
  ...
  if (server.suspended) {
    const err = new Error("Server is suspended — contact support or renew your plan") as Error & {
      code?: string;
    };
...
export async function assertLicensePanelQuota(
  ...
  const memoryAfter = others.reduce((sum, s) => sum + s.memoryMb, 0) + memoryMb;

  if (maxServers != null && opts.extraServer && serverCountAfter > maxServers) {
    throw licenseQuotaError(`License server limit reached (${servers.length}/${maxServers}).`);
  }
