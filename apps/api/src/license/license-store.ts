...
let serverUrlOverride: string | null | undefined;
let licenseKeyClearedHandler: ((state: LicenseState) => Promise<void>) | undefined;
...
function defaultLicenseServerUrl(): string {
  ...
  const publicLicense =
    ...
    (process.env.PUBLIC_HOST?.trim() && process.env.PUBLIC_HOST.trim() !== "localhost"
...
export function setCachedLicenseState(state: LicenseState, at = Date.now()): LicenseState {
  cached = state;
  cachedAt = at;
  return state;
}
