...
import type { LicensePanelStatus, LicenseState } from "./license-store.js";
import { maybeEnforceTransition, wasLicenseValid } from "./validate-effects.js";
...
function allowUnsigned(): boolean {
  if (process.env.LICENSE_ALLOW_UNSIGNED !== "1" && process.env.LICENSE_ALLOW_UNSIGNED !== "true") {
    return false;
  }
...
async function pushDaemonLicenseTicket(state: LicenseState): Promise<void> {
  ...
  if (state.valid && lastSignedTicket && lastSignedTicket.claims.valid) {
    ticket = {
      v: 1,
      kind: "licensed",
      pushedAt: Date.now(),
  ...
  } else if (state.status === "unreachable" && state.valid && lastSignedTicket?.claims.valid) {
  ...
  try {
    ...
    console.warn("[license] daemon ticket push error:", err instanceof Error ? err.message : err);
  }
}
...
export async function validateLicense(force = false): Promise<LicenseState> {
  ...
  if (!force && cache.state && now - cache.cachedAt < LICENSE_CACHE_MS) {
    return cache.state;
  }
...
async function doValidateLicense(): Promise<LicenseState> {
  ...
  try {
    ...
    if (!signatureOk) {
      if (!allowUnsigned()) {
        ...
        if (!pub) {
          ...
          message = `License response from ${base} is missing claims/signature (HTTP ${res.status}) — refusing to trust unsigned status`;
        } else {
          message =
    ...
    cached = {
      ...
      maxServers: data.maxServers === undefined ? null : data.maxServers,
      maxNodes: data.maxNodes === undefined ? null : data.maxNodes,
      maxMemoryMb: data.maxMemoryMb === undefined ? null : data.maxMemoryMb,
      maxMemoryMbPerServer:
        data.maxMemoryMbPerServer === undefined ? null : data.maxMemoryMbPerServer,
      features: normalizeLicenseFeatures(data.features === undefined ? null : data.features),
      boundIp: data.boundIp ?? null,
      boundIps: Array.isArray(data.boundIps) ? data.boundIps : data.boundIp ? [data.boundIp] : [],
    };
    setCachedLicenseState(cached, now);
    ...
    const withinGrace = lastOk != null && Date.now() - lastOk < UNREACHABLE_GRACE_MS;
    // Soft-valid only inside grace after a previously successful signed check.
    const softValid = withinGrace && wasLicenseValid() === true && cached?.valid === true;
    cached = {
      valid: softValid,
      status: "unreachable",
      message: softValid
  ...
  if (cached.valid) {
    ...
    await stopServersExceedingLicenseQuota(cached).catch((err) => {
      console.warn("[license] quota enforcement failed:", err instanceof Error ? err.message : err);
    });
  } else {
