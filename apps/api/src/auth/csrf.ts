...
/**
 * Issue CSRF and persist it. Required because @fastify/session may skip the
 * end-of-request save when `cookie.secure` and the API sees plain HTTP behind
 * prod-web — otherwise settings POSTs fail with "Missing CSRF token".
 */
export async function ensureSessionCsrfToken(request: FastifyRequest): Promise<string> {
  const session = request.session as SessionWithCsrf;
  const had = Boolean(session?.csrfToken?.trim());
...
export function registerCsrfGuard(app: FastifyInstance): void {
  app.addHook("preHandler", async (request, reply) => {
    ...
    const session = request.session as SessionWithCsrf;
    if (!session?.authenticated && (request.apiKeyAuth || request.applicationAuth)) {
      return;
    }
