...
export function quotasForCreate(
  ...
  return {
    maxServers:
      overrides?.maxServers !== undefined
        ? overrides.maxServers
        : defaults.maxServers,
    maxMemoryMb:
      overrides?.maxMemoryMb !== undefined
        ? overrides.maxMemoryMb
        : defaults.maxMemoryMb,
    maxDatabases:
      overrides?.maxDatabases !== undefined
        ? overrides.maxDatabases
        : defaults.maxDatabases,
  };
}
...
export function displayQuotasForRole(
  role: UserRole,
...
export const applicationQuotaLimitSchema = z
  .number()
  .int()
  .min(0)
  .max(100_000)
