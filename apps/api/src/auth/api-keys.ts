...
export async function checkApiKeyRate(keyId: string): Promise<string | null> {
  ...
  const result = await getRateLimitStore().hit(`apikey:${keyId}`, API_KEY_RATE_WINDOW_MS, limit);
  if (result.limited) {
...
export async function resolveApiKeyAuth(
  ...
  const required = (process.env.TWO_FACTOR_REQUIRED_ROLES ?? "")
    ...
    .filter(Boolean);
  if (required.includes(String(row.user.role).toUpperCase()) && !row.user.totpEnabled) {
  ...
  if (limited) {
    (request as FastifyRequest & { apiKeyRateLimited?: string }).apiKeyRateLimited = limited;
    return null;
  }
...
export function apiKeyRateLimitedMessage(request: FastifyRequest): string | null {
  return (request as FastifyRequest & { apiKeyRateLimited?: string }).apiKeyRateLimited ?? null;
}

/** Whether this API key may touch the given server id. */
export function apiKeyAllowsServer(ctx: ApiKeyAuthContext | undefined, serverId: string): boolean {
  if (!ctx) return true;
