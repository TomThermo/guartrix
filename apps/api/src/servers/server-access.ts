...
export async function getServerPermissionsBatch(
  ...
  const ceiling = (granted: string[]) => applyLicenseFeatureCeiling(granted, features);
  ...
  const dbUser = await prisma.user.findUnique({ where: { id: user.id } });
  const email = dbUser?.emailVerified && dbUser.email ? dbUser.email.trim().toLowerCase() : null;
...
export function isServerOwner(user: AuthUser, server: Pick<Server, "ownerId">): boolean {
  if (user.role === "ADMIN") return true;
...
export async function linkPendingSubUsers(
  userId: string,
...
export async function listVisibleServers(
  ...
  const offset =
    opts?.offset != null && Number.isFinite(opts.offset) ? Math.max(0, Math.floor(opts.offset)) : 0;
