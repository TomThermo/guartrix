...
const DEFAULT_ALLOW_PREFIXES = ["/var/lib/guartrix/shared", "/opt/guartrix/shared"];
...
export function parseExtraMounts(input: unknown): ServerExtraMount[] | null {
  ...
  for (let i = 0; i < input.length; i++) {
    ...
    const container = normalizeAbsolutePath(rec.container, `extraMounts[${i}].container`);

    if (container === "/data" || container.startsWith("/data/")) {
      throw new Error(`extraMounts[${i}].container cannot use /data (reserved for server files)`);
    }
