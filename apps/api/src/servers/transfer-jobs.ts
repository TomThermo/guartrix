...
export async function clearPersistedTransferJob(serverId: string): Promise<void> {
...
function jobFromDisk(raw: unknown): TransferJob | null {
  ...
  return {
    ...
    bytesTransferred: typeof j.bytesTransferred === "number" ? j.bytesTransferred : null,
...
export async function hydrateTransferJobsFromDisk(): Promise<void> {
  ...
  try {
    const { getRedis, scanRedisKeys, TRANSFER_KEY_PREFIX } = await import("../redis.js");
    const redis = await getRedis();
    if (redis) {
...
export function getTransferJobInMemory(serverId: string): TransferJob | undefined {
...
export function scheduleTransferJobCleanup(serverId: string, delayMs = 30 * 60_000): void {
  setTimeout(() => {
