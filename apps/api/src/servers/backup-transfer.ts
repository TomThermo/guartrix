...
async function readMeta(serverId: string, uploadId: string): Promise<UploadMeta> {
...
export async function initChunkedUpload(opts: {
  ...
  if (sizeBytes > BACKUP_UPLOAD_MAX_BYTES) {
    throw new Error(`Backup too large (max ${formatBackupSize(BACKUP_UPLOAD_MAX_BYTES)})`);
  }
...
export async function getChunkedUploadStatus(
  ...
  const receivedChunks = await listReceivedChunks(serverId, uploadId, meta.totalChunks);
  let receivedBytes = 0;
...
export async function saveUploadChunk(opts: {
  ...
  if (opts.data.length !== expected) {
    throw new Error(`Chunk ${index} size mismatch: expected ${expected}, got ${opts.data.length}`);
  }
  ...
  const receivedChunks = await listReceivedChunks(opts.serverId, opts.uploadId, meta.totalChunks);
  let receivedBytes = 0;
  for (const idx of receivedChunks) {
    receivedBytes +=
...
export async function completeChunkedUpload(opts: {
  ...
  const received = await listReceivedChunks(opts.serverId, opts.uploadId, meta.totalChunks);
  if (received.length !== meta.totalChunks) {
    throw new Error(`Upload incomplete: ${received.length}/${meta.totalChunks} chunks`);
  }
  ...
  const partialPath = path.join(sessionDir(opts.serverId, opts.uploadId), "assembled.partial");
  ...
  if (st.size !== meta.sizeBytes) {
    await fs.rm(partialPath, { force: true }).catch(() => undefined);
    throw new Error(`Assembled size mismatch: expected ${meta.sizeBytes}, got ${st.size}`);
  }
...
export async function abortChunkedUpload(serverId: string, uploadId: string): Promise<void> {
  await fs
