...
export async function decryptBackupArchive(encPath: string, plainOutPath: string): Promise<void> {
  const st = await fsp.stat(encPath);
  ...
  try {
    ...
    await pipeline(input, transform, fs.createWriteStream(plainOutPath, { mode: 0o600 }));
  } finally {
