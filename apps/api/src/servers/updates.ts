...
const channelCache = new Map<
  string,
  { at: number; label: string | null; raw: string | number | null }
>();
...
export async function checkServerUpdate(server: Server): Promise<ServerUpdateInfo> {
  ...
  const mcUpdateAvailable =
    versions.length > 0 && compareVersionsAsc(server.mcVersion, latestMcVersion) < 0;
  ...
  if (server.type === "PAPER") {
    ...
    if (latestBuild != null) {
      ...
      channelUpdateAvailable = server.paperBuild == null || latestBuild > server.paperBuild;
    }
    ...
    if (latestForge) {
      ...
      channelUpdateAvailable =
        !server.forgeVersion || compareVersionsAsc(currentBuild, latestBuild) < 0;
    }
    ...
    if (latestNeo) {
      ...
      channelUpdateAvailable =
        !server.forgeVersion || compareVersionsAsc(server.forgeVersion, latestNeo) < 0;
    }
    ...
    if (latest) {
      ...
      channelUpdateAvailable = compareVersionsAsc(server.mcVersion, latest) < 0;
    }
