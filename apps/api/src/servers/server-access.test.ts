...
describe("parsePermissionsJson", () => {
  ...
  it("keeps known server permissions and drops junk", () => {
    expect(
      parsePermissionsJson(JSON.stringify(["control.start", "file.read", "not-a-perm", 42, null])),
...
describe("serializePermissions", () => {
  it("dedupes and filters invalid entries", () => {
    const raw = serializePermissions(["control.start", "control.start", "file.read", "nope"]);
  ...
  it("round-trips with parsePermissionsJson", () => {
    ...
    expect(parsePermissionsJson(serializePermissions(perms)).sort()).toEqual([...perms].sort());
  });
});
...
describe("subuser access", () => {
  ...
  it("links and returns a pending email invite", async () => {
    ...
    mocks.subUserFindFirst.mockResolvedValueOnce(null).mockResolvedValueOnce({
      id: "sub-2",
      userId: null,
...
describe("permission resolution", () => {
  ...
  it("uses subuser permissions and applies the license ceiling", async () => {
    ...
    await expect(getServerPermissions(guest, server)).resolves.toEqual(["file.read"]);
  });
  ...
  it("checks access, ownership, and required permissions", async () => {
    ...
    await expect(userHasServerPermission(guest, server, "file.read")).resolves.toBe(true);
    expect(hasServerPermission({ permissions: ["file.read"] }, "file.read")).toBe(true);
  });
});

describe("batch permissions", () => {
  const servers = [server, { id: "srv-2", ownerId: "someone-else" }];
...
describe("access helpers and visible server lists", () => {
  ...
  it("lists visible server ids and applies API-key scope", async () => {
    mocks.serverFindMany.mockResolvedValue([{ id: "srv-1" }, { id: "srv-2" }]);
    await expect(listVisibleServerIds(admin)).resolves.toEqual(["srv-1", "srv-2"]);
    await expect(
