...
export async function writeFileContent(serverId: string, relPath: string, content: string) {
...
export async function compressFiles(serverId: string, paths: string[], destination: string) {
...
export async function decompressFile(serverId: string, archivePath: string, destination?: string) {
...
export async function saveUpload(
  ...
  const { resolveNodeForServer, daemonFetch } = await import("../nodes/daemon-client.js");
  const tmp = path.join(
    os.tmpdir(),
  ...
  try {
    ...
    if (st.size > maxBytes) {
      throw new Error(`File too large (max ${Math.round(maxBytes / (1024 * 1024))} MiB)`);
    }
    const form = new FormData();
    form.append("file", await openAsBlob(tmp), filename);
    const q = new URLSearchParams({ path: destDir });
    const res = await daemonFetch(node, `/servers/${serverId}/files/upload?${q}`, {
      method: "POST",
      body: form,
    });
