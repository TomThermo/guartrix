...
export async function assertCanCreateServer(
  ...
  if (user.maxServers != null && usage.serverCount >= user.maxServers) {
    ...
    throw new Error(
      `Server limit reached (${usage.serverCount}/${user.maxServers}).`,
    );
  }
...
export async function assertCanAllocateMemory(
  ...
  if (opts?.extraServer) {
    ...
    if (user.maxServers != null && serverCount > user.maxServers) {
      ...
      throw new Error(
        `Server limit reached (${servers.length}/${user.maxServers}).`,
      );
    }
  }
...
export function formatQuotaLabel(user: AuthUser): string {
  const servers =
    user.maxServers == null
      ? "∞ servers"
      : `${user.serverCount ?? 0}/${user.maxServers} servers`;
  const ram =
  ...
  const dbs =
    user.maxDatabases == null
      ? "∞ DBs"
      : `${user.databaseCount ?? 0}/${user.maxDatabases} DBs`;
