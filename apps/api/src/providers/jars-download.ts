...
export async function downloadFabric(
  ...
  const installer = installerVersions.find((i) => i.stable) ?? installerVersions[0];
...
async function finishInstallerRuntime(
  ...
  const runtimeJar = entries.find(
    (name) => name.startsWith(jarPrefix) && name.endsWith(".jar") && !name.includes("installer"),
  );
  if (!runtimeJar) {
    throw new Error(`${label} install finished but no run.sh or ${jarPrefix}jar was found`);
  }
