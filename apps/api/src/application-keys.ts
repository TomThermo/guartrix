...
export function applicationRateLimitedMessage(
  request: FastifyRequest,
): string | null {
  return (
    (request as FastifyRequest & { applicationRateLimited?: string })
      .applicationRateLimited ?? null
  );
}

export function applicationHasScope(
  ctx: ApplicationAuthContext,
  scope: string,
): boolean {
