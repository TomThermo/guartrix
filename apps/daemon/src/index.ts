...
import { acceptLicenseTicket, getLicenseTicketStatus } from "./license-gate.js";
...
async function main() {
  ...
  app.get<{
    ...
    const includeDisk =
      request.query.disk === "1" || request.query.disk === "true" || request.query.disk === "";
  ...
  app.get<{ Params: { id: string } }>("/servers/:id/disk", async (request) =>
    getDiskUsageCached(request.params.id),
  );

  app.get<{ Params: { id: string } }>("/servers/:id/players", async (request) => ({
    online: processManager.getOnlinePlayerNames(request.params.id),
  }));

  app.get<{ Params: { id: string } }>("/servers/:id/history", async (request) => ({
  ...
  app.post<{
    ...
    const protocol = request.body?.protocol === "udp" ? "udp" : ("tcp" as const);
    const free = await processManager.isPortFree(port, protocol);
    return { free };
  });

  app.post<{ Params: { id: string } }>("/servers/:id/chown", async (request, reply) => {
    try {
  ...
  app.get<{ Params: { id: string } }>("/servers/:id/running", async (request) => {
  ...
  app.log.info(`Guartrix daemon listening on http://${daemonConfig.host}:${daemonConfig.port}`);
  ...
  void (async () => {
    try {
      ...
      app.log.warn(err, "Bedrock runtime image prebuild failed — will retry on server start");
    }
  })();
