...
export function registerMysqlRoutes(app: FastifyInstance): void {
  ...
  app.get("/mysql/logs/stream", { websocket: true }, (socket, request) => {
    ...
    const child = spawn("sudo", ["-n", "docker", "logs", "-f", "--tail", "0", MYSQL_CONTAINER], {
      env: { ...process.env },
    });
  ...
  app.post<{ Body: { name?: string } }>("/mysql/databases/dump", async (request, reply) => {
    const name = request.body?.name?.trim();
    if (!name) return reply.status(400).send({ error: "name is required" });
    const tmp = path.join(os.tmpdir(), `guartrix-mysqldump-${name}-${Date.now()}.sql`);
    try {
  ...
  app.post<{ Querystring: { name?: string } }>(
    ...
    async (request, reply) => {
      ...
      const tmp = path.join(os.tmpdir(), `guartrix-mysqlrestore-${name}-${Date.now()}.sql`);
      try {
  ...
  app.post<{
    ...
    const tmp = path.join(os.tmpdir(), `guartrix-mysql-peer-${name}-${Date.now()}.sql`);
    try {
      ...
      if (!res.ok) {
        ...
        return reply.status(502).send({ error: text || `Source dump failed (${res.status})` });
      }
