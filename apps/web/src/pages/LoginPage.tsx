...
export function LoginPage() {
  ...
  async function onSubmitPassword(e: FormEvent) {
    ...
    try {
      const result = await login(username, password, rememberMe, turnstileToken || undefined);
      if (rememberMe) {
  ...
  if (needsTwoFactor) {
    return (
      <AuthShell title={t("auth.twoFactorTitle")} subtitle={t("auth.twoFactorSubtitle")}>
        {error && (
        ...
        <Form onSubmit={onSubmitCode}>
          <Form.Group className="mb-3" controlId="totp-code">
            ...
            <Form.Control
              ...
              autoFocus
            />
            <Form.Text className="text-secondary">{t("auth.recoveryHint")}</Form.Text>
          </Form.Group>
  ...
  return (
    <AuthShell title={t("auth.signInTitle")} subtitle={t("auth.signInSubtitle")}>
      ...
      <div className="d-flex justify-content-between flex-wrap gap-2 mt-3 small">
        ...
        {registrationEnabled && <Link to="/register">{t("auth.createAccount")}</Link>}
      </div>
    </AuthShell>
  );
}
