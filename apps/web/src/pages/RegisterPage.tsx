    ...
    return (
      ...
      >
        <Alert variant="secondary" className="mb-0">
          {t("auth.registrationClosedBody")} <Link to="/login">{t("auth.signIn")}</Link>.
        </Alert>
      </AuthShell>
    );
  }
    ...
    <AuthShell title={t("auth.registerTitle")} subtitle={t("auth.registerSubtitle")}>
      ...
      <Form onSubmit={onSubmit}>
        ...
        <Form.Check
          ...
          label={
            <span>
              {t("auth.agreeTermsPrefix")} <Link to="/terms">{t("auth.termsOfService")}</Link>{" "}
              {t("auth.and")} <Link to="/privacy">{t("auth.privacyPolicy")}</Link>
            </span>
          }
        />
