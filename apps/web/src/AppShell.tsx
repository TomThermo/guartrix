...
export function AppShell({ children }: { children: ReactNode }) {
  ...
  const needsTwoFactor = Boolean(user?.twoFactorRequired) && !user?.twoFactorEnabled;
  ...
  const [licenseBannerDismissed, setLicenseBannerDismissed] = useState(() => {
    try {
      const until = Number(localStorage.getItem("guartrix.licenseBannerDismissedUntil") || "0");
  ...
  useEffect(() => {
    ...
    const intervalMs = !licenseOk ? 30_000 : onLicensePage ? 60_000 : 10 * 60_000;
  ...
  function dismissLicenseBanner() {
    ...
    try {
      localStorage.setItem("guartrix.licenseBannerDismissedUntil", String(until));
    } catch {
      /* ignore */
    }
    ...
    <div className="app-shell">
            ...
            <div className="app-nav-actions ms-md-auto mt-3 mt-md-0">
              ...
              <Dropdown align="end" className="app-nav-dropdown">
                ...
                <Dropdown.Menu className="app-nav-dropdown-menu">
                  <Dropdown.Item as={Link} to="/wiki" onClick={closeNav}>
                    <i className="fa-solid fa-book-open fa-fw me-2 text-secondary" />
                    {t("nav.wiki")}
                  </Dropdown.Item>
                  <Dropdown.Item as={Link} to="/api-docs" onClick={closeNav}>
                    <i className="fa-solid fa-code fa-fw me-2 text-secondary" />
                    {t("nav.api")}
                  </Dropdown.Item>
                </Dropdown.Menu>
              </Dropdown>
              ...
              {isAdmin && (
                ...
                >
                  ...
                  {t("nav.admin")}
                  {!licenseOk && (
                    <span className="app-nav-alert-dot" title={t("nav.licenseIssue")} />
                  )}
                </Link>
              )}
              ...
              {user && (
                <Dropdown align="end" className="app-nav-dropdown">
                  ...
                  >
                    ...
                    {needsTwoFactor && (
                      <span className="app-nav-alert-dot" title={t("nav.twoFactorRequired")} />
                    )}
                  </Dropdown.Toggle>
                  <Dropdown.Menu className="app-nav-dropdown-menu app-nav-account-menu">
                    <Dropdown.Header>
                      {t("nav.signedInAs", { username: user.username })}
                    </Dropdown.Header>
                    <Dropdown.Header className="app-nav-dropdown-section">
                      {t("nav.accountMenuPreferences")}
                    </Dropdown.Header>
                    <Dropdown.Item
                      as={Link}
                      to="/account/security"
                      onClick={closeNav}
                    >
                      <i className="fa-solid fa-id-card fa-fw me-2 text-secondary" />
                      {t("account.tabProfile")}
                    </Dropdown.Item>
                    <Dropdown.Item
                      as={Link}
                      to="/account/security?tab=appearance"
                      onClick={closeNav}
                    >
                      <i className="fa-solid fa-palette fa-fw me-2 text-secondary" />
                      {t("account.tabAppearance")}
                    </Dropdown.Item>
                    <Dropdown.Header className="app-nav-dropdown-section">
                      {t("nav.accountMenuSecurity")}
                    </Dropdown.Header>
                    <Dropdown.Item
                      as={Link}
                      to="/account/security?tab=security"
                      onClick={closeNav}
                    ...
                    <Dropdown.Item
                      as={Link}
                      to="/account/security?tab=access"
                      onClick={closeNav}
                    >
                    ...
                    <Dropdown.Item
                      as={Link}
                      to="/account/security?tab=notifications"
                      onClick={closeNav}
                    >
                      <i className="fa-solid fa-bell fa-fw me-2 text-secondary" />
                      {t("account.tabNotifications")}
                    </Dropdown.Item>
                    <Dropdown.Item
                      as={Link}
                      to="/account/security?tab=privacy"
                      onClick={closeNav}
                    >
                      <i className="fa-solid fa-user-lock fa-fw me-2 text-secondary" />
                      {t("account.tabPrivacy")}
                    </Dropdown.Item>
                    <Dropdown.Header className="app-nav-dropdown-section">
                      {t("nav.accountMenuBilling")}
                    </Dropdown.Header>
                    <Dropdown.Item as={Link} to="/account/billing" onClick={closeNav}>
      ...
      <Container className={`app-main ${widthClass}`.trim()}>
        ...
        {isAdmin && !licenseOk && !licenseBannerDismissed && (
          <Alert variant="danger" className="mt-3 mb-0" dismissible onClose={dismissLicenseBanner}>
            <strong>License issue.</strong> {licenseMsg || "License is not valid."}
            {licenseMsg && !/[.!?]$/.test(licenseMsg.trim()) ? "." : ""} Free tier applies: 1 node,
            1 server, 10 GB disk. Servers beyond those caps are stopped.{" "}
        ...
        {needsTwoFactor && location.pathname !== "/account/security" && (
          <Alert variant="warning" className="mt-3 mb-0">
            ...
            <Link to="/account/security?tab=security" className="alert-link">
              Enable it under Account → Security
            </Link>{" "}
