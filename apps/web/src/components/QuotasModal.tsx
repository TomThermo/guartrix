...
export function QuotasModal({ user, hostMemoryGb, onCancel, onSaved }: Props) {
  ...
  const defaultGb = useMemo(() => {
    ...
    return Math.min(hostMemoryGb, Math.max(1, Math.round(user.maxMemoryMb / 1024)));
  ...
  const [unlimitedDatabases, setUnlimitedDatabases] = useState(user.maxDatabases == null);
