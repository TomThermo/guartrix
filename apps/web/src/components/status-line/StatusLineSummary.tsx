...
export function StatusLineOverview({ data }: { data: AdminStatusResponse }) {
  ...
  const gameRunning = data.nodes.reduce(
    (sum, n) =>
      sum +
      n.containers.filter((c) => !c.isMysql && c.state.toLowerCase() === "running")
        .length,
    0,
  );
  const mysqlOk = data.nodes.every((n) => !n.mysql || n.mysql.running);
  const sftpOk = data.nodes.every(
    (n) => !n.sftp || (n.reachable && n.sftp.listening),
  );

  const redisOk = !panel?.redis?.enabled
    ? true
    : Boolean(panel.redis.connected);
      ...
      <section className="admin-inset-card">
        ...
        <div className="status-tile-grid">
          <StatusTile
            ...
            statusLabel={
              panel?.web?.ok ? t("common.online") : t("admin.statusOffline")
            }
            rows={[
