...
import type { PortAllocation, ServerType } from "@guartrix/shared";
...
export function AllocationsPanel({
  ...
  const showGeyser = canInstallAddons && (serverType === "PAPER" || serverType === "PURPUR");
  ...
  useEffect(() => {
    setLoading(true);
    void refresh()
      .catch((err) => onError(err instanceof Error ? err.message : t("allocations.loadFailed")))
      .finally(() => setLoading(false));
  }, [refresh, onError, t]);

  async function installGeyser() {
    if (!showGeyser) return;
    if (!confirm(t("allocations.geyserConfirm"))) {
      return;
    }
    setGeyserBusy(true);
    onError(null);
  ...
  async function onCreatePort(e: FormEvent) {
    ...
    try {
      ...
      setPort("");
      onNotice(t("allocations.noticePortAssigned", { port: n, protocol }));
      await refresh();
    } catch (err) {
  ...
  return (
    <div className="databases-panel">
      ...
      <Alert variant="warning" className="mb-3">
        <strong>{t("allocations.restartRequired")}</strong> {t("allocations.restartHelp")}
      </Alert>
            ...
            <section className="databases-section">
              ...
              {free.length > 0 && (
                <Form onSubmit={onAssignFree} className="mb-3">
                  ...
                  <Button type="submit" size="sm" disabled={busy || !pickId} variant="primary">
              ...
              <Form onSubmit={onCreatePort}>
                ...
                <Form.Select
                  size="sm"
                  value={protocol}
                  onChange={(e) => setProtocol(e.target.value === "udp" ? "udp" : "tcp")}
                  className="mb-2"
                >
