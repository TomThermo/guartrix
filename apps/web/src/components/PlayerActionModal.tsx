...
import type { OnlinePlayer, PlayerModerationEvent } from "@guartrix/shared";
import {
  Badge,
  Button,
  Form,
...
const ONLINE_ONLY = new Set<PlayerAction>(["kick", "whisper", "give", "gamemode", "kill", "clear"]);
          ...
          <>
            {!online && (
              <p className="small text-secondary">
                Player is offline — kick, whisper, give, gamemode, kill and clear are disabled. Ban,
                OP and whitelist still work when the server is running.
              </p>
            )}
            ...
            <Stack direction="horizontal" gap={2} className="flex-wrap mb-2">
              <Button
                ...
                disabled={busy || !can("kick")}
                onClick={() => void run("kick", { reason })}
              >
                {t("players.kick")}
              </Button>
              <Button
              ...
              <Button
                size="sm"
                variant="outline-secondary"
                disabled={busy || !can("pardon")}
                onClick={() => void run("pardon")}
              >
                Unban
              </Button>
              <Button
            ...
            <Stack direction="horizontal" gap={2} className="flex-wrap mb-3">
              ...
              <Button
                size="sm"
                variant="outline-secondary"
                disabled={busy || !can("deop")}
                onClick={() => void run("deop")}
              >
                {t("players.deop")}
              </Button>
              <Button
                size="sm"
                variant="outline-secondary"
                disabled={busy || !can("whitelist_add")}
                onClick={() => void run("whitelist_add")}
              >
                Whitelist
              </Button>
              <Button
                size="sm"
                variant="outline-secondary"
                disabled={busy || !can("whitelist_remove")}
                onClick={() => void run("whitelist_remove")}
              >
                Unwhitelist
              </Button>
            </Stack>
            ...
            <InputGroup size="sm" className="mb-3" style={{ maxWidth: 320 }}>
              ...
              <Button
                variant="primary"
                disabled={busy || !can("gamemode")}
                onClick={() => void run("gamemode", { gamemode })}
              >
                Set
              </Button>
            </InputGroup>
            ...
            <Form onSubmit={(e) => void onWhisper(e)} className="mb-3">
              <InputGroup size="sm">
                ...
                <Button
                  type="submit"
                  variant="primary"
                  disabled={busy || !can("whisper") || !whisper.trim()}
                >
                  Send
                </Button>
              </InputGroup>
