...
export function ServerConsoleLayout({
  ...
  const toggleCharts = () => {
    setShowCharts((prev) => {
      const next = !prev;
      try {
        localStorage.setItem("guartrix.consoleCharts", next ? "1" : "0");
      } catch {
        /* ignore */
      }
      return next;
  ...
  const pushSample = useCallback(
    (nextStats: ServerStats) => {
      ...
      const sample: ChartSample = {
        ...
        memoryMb: nextStats.running ? Math.round(nextStats.memoryUsedBytes / (1024 * 1024)) : 0,
  ...
  }, [server.id, pushSample, showCharts]);
      ...
      <Col xs={12}>
        <section className="console-metrics" aria-label={t("console.metricsTitle")}>
          <div className="console-metrics__head">
            <h3 className="console-metrics__title mb-0">{t("console.metricsTitle")}</h3>
            <Button
              size="sm"
              variant="outline-secondary"
            ...
            >
              <i
                className={`fa-solid ${showCharts ? "fa-chevron-up" : "fa-chevron-down"} me-1`}
                aria-hidden
              />
              {showCharts ? t("console.hideCharts") : t("console.showCharts")}
            </Button>
          </div>
          {showCharts ? (
            <>
              <span className="console-metrics__hint d-block mb-2">{t("console.metricsHint")}</span>
            ...
            <p className="small text-secondary mb-0 mt-1">{t("console.chartsCollapsedHint")}</p>
          )}
        </section>
      </Col>
    </Row>
