...
export function DatabasesPanel({
  ...
  useEffect(() => {
    setLoading(true);
    void refresh()
      .catch((err) => onError(err instanceof Error ? err.message : t("databases.loadFailed")))
  ...
  return (
    <div className="databases-panel">
      <header className="databases-panel-header">
        ...
        <p className="databases-panel-lead mb-0">All databases available for this server.</p>
      </header>
      ...
      <Row className="g-3 databases-layout">
        <Col xs={12} lg={9}>
          <section className="databases-section h-100">
            ...
            {databases.length === 0 ? (
              <p className="databases-empty mb-0">{t("databases.empty")}</p>
            ) : (
              ...
              <Form onSubmit={onCreate} className="databases-create-form">
                <Form.Group className="mb-3">
                  ...
                  <InputGroup>
                    ...
                    <Form.Control
                      value={suffix}
                      onChange={(e) => setSuffix(e.target.value.replace(/[^a-zA-Z0-9_]/g, ""))}
                  ...
                  <Form.Text className="databases-hint">
                    You are currently using {used} of {limitLabel} databases. A username and
                    password for this database will be randomly generated after form submission.
                  </Form.Text>
                </Form.Group>

                <Alert variant="warning" className="databases-warn small mb-3">
                  Please do not use your database for Dynmap or Coreprotect. This does not improve
                  performance.
                </Alert>
