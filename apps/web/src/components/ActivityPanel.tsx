...
export function ActivityPanel({ serverId, userId, showServer = false, onError }: Props) {
  const { t } = useI18n();
  ...
  useEffect(() => {
    setLoading(true);
    void load(0)
      .catch((err) => onError(err instanceof Error ? err.message : t("activity.loadFailed")))
  ...
  function exportCsv() {
    ...
    downloadBlob(`activity-${Date.now()}.csv`, [headers.join(","), ...rows].join("\n"), "text/csv");
  }
      ...
      <Form onSubmit={submitSearch}>
        <Row className="g-2 align-items-end mb-3">
          ...
          <Col xs={6} md={3}>
            ...
            <Form.Select size="sm" value={action} onChange={(e) => setAction(e.target.value)}>
            ...
            <tbody>
              {events.map((event) => (
                <tr key={event.id}>
                  ...
                  <td className="text-secondary small">{activityDetail(event) || "—"}</td>
                  <td className="text-secondary small font-monospace">{event.actorIp ?? "—"}</td>
                </tr>
              ))}
            </tbody>
          </Table>
