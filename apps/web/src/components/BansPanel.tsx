...
type EditTarget =
  | { kind: "player"; ban: BanEntry }
  | { kind: "ip"; ban: IpBanEntry }
  | null;
...
export function BansPanel({
  ...
  async function removePlayer(name: string) {
    ...
    try {
      ...
      onNotice(
        serverRunning
          ? `Unbanned ${name} (pardon sent to server).`
          : `Removed ban for ${name}.`,
      );
    } catch (err) {
  ...
  async function saveEdit(e: FormEvent) {
    ...
    try {
      ...
      onNotice(
        serverRunning
          ? "Ban updated and re-applied on the live server."
          : "Ban updated on disk.",
      );
    } catch (err) {
          ...
          >
            ...
            {canUpdate && (
              <Stack direction="horizontal" gap={2}>
                <Button size="sm" variant="outline-secondary" disabled={busy} onClick={() => openEditPlayer(ban)}>
                  Edit
                </Button>
      ...
      <ListGroup>
        ...
        {bans.ips.map((ban) => (
          ...
          >
            ...
            {canUpdate && (
              <Stack direction="horizontal" gap={2}>
                <Button size="sm" variant="outline-secondary" disabled={busy} onClick={() => openEditIp(ban)}>
                  Edit
                </Button>
