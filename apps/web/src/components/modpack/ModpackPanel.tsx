...
import { AddonDetailModal } from "../addon-panel/AddonDetailModal";
import { AbortableBrowse } from "../addon-panel/abortableBrowse";
import { AddonVersionPickerModal } from "../addon-panel/AddonVersionPickerModal";
import { ModpackSearch } from "./ModpackSearch";
import { normalizeModpackHit, type ModpackHit, type ModpackSource } from "./normalizeHit";
...
export function ModpackPanel({ server, canUpdate, onNotice, onError }: Props) {
  const { t } = useI18n();
    ...
    async (nextOffset = 0, append = false) => {
      ...
      try {
        const res = await api.searchModpacks(server.id, {
          q: query,
          source,
          category: source === "modrinth" && category ? category : undefined,
  ...
  }, [supports, source, category, browse]);
