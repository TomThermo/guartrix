...
export function EngineSettingsPanel({ server, canUpdate, onNotice, onError }: Props) {
  const { t } = useI18n();
  ...
  if (!supported) {
    return (
      <Alert variant="light" className="border">
        Guided engine settings are available for Paper and Purpur. For Fabric/Forge, edit files
        under <code>config/</code> in the File Manager.
      </Alert>
    );
  }
  ...
  return (
    <div>
      ...
      <p className="text-secondary small">
        Common Paper/Spigot/Purpur toggles. Advanced options: open the YAML in File Manager.
      </p>
          ...
          <Form.Group key={f.id} controlId={`engine-${f.id}`}>
            ...
            {f.type === "boolean" ? (
              <Form.Check
                ...
                disabled={!canUpdate}
                onChange={(e) => setDraft((d) => ({ ...d, [f.id]: e.target.checked }))}
              />
            ) : (
              <Form.Control
                type={f.type === "number" ? "number" : "text"}
                value={draft[f.id] === undefined || draft[f.id] === null ? "" : String(draft[f.id])}
                disabled={!canUpdate}
                onChange={(e) =>
                  setDraft((d) => ({
                    ...d,
