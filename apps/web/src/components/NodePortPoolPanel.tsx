...
export function NodePortPoolPanel({
  ...
  useEffect(() => {
    setLoading(true);
    void refresh()
      .catch((err) => onError(err instanceof Error ? err.message : t("admin.loadPortPoolFailed")))
    ...
    try {
      ...
      onNotice(
        t("admin.createRangeNotice", {
          ...
          node: nodeName,
          skipped: result.skipped ? t("admin.createRangeSkipped", { skipped: result.skipped }) : "",
        }),
      );
      await refresh();
    } catch (err) {
            ...
            <tbody>
              {allocations.map((a) => (
                <tr key={a.id}>
                  ...
                  <td>
                    {a.serverId ? (
                      <Badge bg="secondary">{a.serverName ?? t("admin.assignedStatus")}</Badge>
                    ) : (
