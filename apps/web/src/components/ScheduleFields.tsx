...
export function ScheduleFields({
  ...
  const whenLabel = labels === "backups" ? t("backups.mode") : t("schedules.timing.when");
  const timeLabel = labels === "backups" ? t("backups.timeLocal") : t("schedules.timing.time");
  const intervalLabel =
    labels === "backups" ? t("backups.intervalHours") : t("schedules.timing.intervalHours");
      ...
      {mode === "cron" && onCronExpressionChange && (
        <Form.Group className="mb-3">
          ...
          <Form.Control
            ...
            required
          />
          <Form.Text className="text-secondary">{t("schedules.timing.cronHint")}</Form.Text>
        </Form.Group>
      )}
    </>
  );
