...
export function JoinCard({ server, connect, compact, onNotice }: Props) {
  ...
  const directIp = connect?.directIp != null ? `${connect.directIp}:${connect.port}` : address;
  const playersOnline = connect?.onlinePlayers ?? 0;
  const playersMax = connect?.playersMax ?? (Number(connect?.maxPlayers ?? 20) || 20);
  ...
  if (compact) {
    return (
      <Stack
        ...
        gap={2}
        className="join-card-compact flex-wrap align-items-center"
      >
