 ...
 */
export function DiskUsageCard({ disk, limitMb, compact = false, className }: Props) {
  const { t } = useI18n();
  ...
  const freeBytes = limitBytes != null ? Math.max(0, limitBytes - usedData) : null;
  const limitLabel = limitMb && limitMb > 0 ? formatGb(limitMb) : null;
      ...
      <Card.Body className={compact ? "py-2 px-3" : undefined}>
        <div className="d-flex justify-content-between align-items-baseline mb-2 flex-wrap gap-1">
          ...
          <strong className={over ? "text-danger" : undefined}>
            {disk.totalLabel}
            {limitLabel ? <span className="text-muted fw-normal"> / {limitLabel}</span> : null}
            {freeBytes != null ? (
        ...
        {over && <div className="text-danger small mb-2">{t("resources.quotaReached")}</div>}
        <div
          className={`disk-usage-bar ${compact ? "mb-2" : "mb-3"}`}
          role="img"
          aria-label={t("resources.breakdownAria")}
          onMouseMove={(e) => setHoverKey(keyAtBarX(e.clientX, e.currentTarget))}
            ...
            <Col key={p.key}>
              ...
              >
                <div className="disk-usage-legend-label">
                  <span className={`disk-usage-swatch disk-usage-swatch-${p.key}`} aria-hidden />
                  {p.label}
                </div>
