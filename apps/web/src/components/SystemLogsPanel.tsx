...
export type SystemLogSource = "daemon" | "api" | "web" | "monitor" | "mysql" | `mc:${string}`;
...
function LiveFileConsole({
  ...
  useEffect(() => {
    ...
    const ws = new WebSocket(`${proto}//${window.location.host}/ws/admin/logs/${source}?tail=400`);
  ...
  return (
    <div className="console-panel status-log-console">
      ...
      >
        ...
        {connected && lines.length === 0 && <div className="text-secondary">{emptyLabel}</div>}
        {lines.map((line, i) => (
