...
export function ProxySetupCard({ server, disabled, onNotice, onError, onApplied }: Props) {
  const { t } = useI18n();
  const [busy, setBusy] = useState(false);
  const [setup, setSetup] = useState<Awaited<ReturnType<typeof api.getProxySetup>> | null>(null);
  ...
  async function apply(mode: "none" | "velocity" | "bungeecord") {
    ...
    try {
      ...
      setSetup(next);
      onNotice(mode === "none" ? t("proxy.clearedNotice") : t("proxy.configuredNotice", { mode }));
      onApplied?.();
    } catch (err) {
