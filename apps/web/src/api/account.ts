...
export const accountApi = {
  ...
  revokeAppPassword: (id: string) =>
    request<{ password: { id: string } }>(`/api/account/app-passwords/${encodeURIComponent(id)}`, {
      method: "DELETE",
    }),
  getAccountProfile: () =>
  ...
  clearPushSubscriptions: () =>
    request<{ ok: boolean; deleted: number }>("/api/account/push/subscriptions", {
      method: "DELETE",
    }),
  ...
  acceptInvite: (token: string) =>
    request<{ ok: boolean; serverId: string }>(`/api/invites/${encodeURIComponent(token)}/accept`, {
      method: "POST",
    }),
  listApplicationKeys: () =>
    request<{ keys: ApplicationApiKeyRecord[]; maxKeys: number }>("/api/admin/application-keys"),
  createApplicationKey: (body: { name: string; scopes: string[]; note?: string | null }) =>
    request<{ key: ApplicationApiKeyRecord; token: string }>("/api/admin/application-keys", {
      method: "POST",
      body: JSON.stringify(body),
    }),
