...
export const filesApi = {
  ...
  deleteFile: (id: string, path: string) =>
    request<void>(`/api/servers/${id}/files?path=${encodeURIComponent(path)}`, {
      method: "DELETE",
    }),
  ...
  downloadFile: async (id: string, path: string, fileName: string) => {
    const res = await fetch(`/api/servers/${id}/files/download?path=${encodeURIComponent(path)}`, {
      credentials: "include",
    });
    if (!res.ok) {
      const data = await res.json().catch(() => ({}));
  ...
  compressFiles: (id: string, paths: string[], destination: string) =>
    request<{ ok: boolean; path: string; size: number }>(`/api/servers/${id}/files/compress`, {
      method: "POST",
  ...
  decompressFile: (id: string, path: string, destination?: string) =>
    request<{ ok: boolean; path: string }>(`/api/servers/${id}/files/decompress`, {
      method: "POST",
      body: JSON.stringify({
        path,
