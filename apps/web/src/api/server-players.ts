...
export const serverPlayersApi = {
  getPlayers: (id: string) => request<PlayersResponse>(`/api/servers/${id}/players`),
  getOnlinePlayers: (id: string) => request<OnlinePlayersResponse>(`/api/servers/${id}/online`),
  getAllOnlinePlayers: () => request<Record<string, OnlinePlayersResponse>>("/api/servers/online"),
  playerAction: (
    id: string,
    body: {
      action: string;
  ...
  removeWhitelist: (id: string, name: string) =>
    request<PlayersResponse>(`/api/servers/${id}/whitelist/${encodeURIComponent(name)}`, {
      method: "DELETE",
    }),
  ...
  removeOp: (id: string, name: string) =>
    request<PlayersResponse>(`/api/servers/${id}/ops/${encodeURIComponent(name)}`, {
      method: "DELETE",
    }),
  ...
  updatePlayerBan: (id: string, name: string, body: { reason?: string; expires?: string }) =>
    request<BansResponse>(`/api/servers/${id}/bans/players/${encodeURIComponent(name)}`, {
      method: "PATCH",
  ...
  updateIpBan: (id: string, ip: string, body: { reason?: string; expires?: string }) =>
    request<BansResponse>(`/api/servers/${id}/bans/ips/${encodeURIComponent(ip)}`, {
      method: "PATCH",
      body: JSON.stringify(body),
    }),
