...
export function t(key: MessageKey | string, params?: TranslateParams): string {
  ...
  const primary = catalogs[currentLocale] ? resolvePath(catalogs[currentLocale]!, key) : undefined;
  const fallback = currentLocale === "en" ? undefined : resolvePath(catalogs.en!, key);
