...
- [x] Optional Cloudflare Turnstile on login / register (**Admin → Security**)
...
| CSP | prod-web sends `script-src` with a **per-request nonce** (stamped on `index.html` scripts). Allows `https://challenges.cloudflare.com` for Turnstile and `static.cloudflareinsights.com` for Web Analytics. `worker-src 'self' blob:` for Monaco editor workers. Prefer turning off **Email Address Obfuscation** (Scrape Shield) rather than `'unsafe-inline'` |
...
| Turnstile | Optional Cloudflare Turnstile on login/register — **Admin → Security** (or `TURNSTILE_*` env); server verifies via siteverify before credentials / account create |
...
| Outbound | Shared host/IP policy (`@guartrix/shared` `safe-url`); API webhook/download SSRF + CDN allowlist + **DNS-pin** fetches (`apps/api/src/safe-url/`); wiki/addon markdown links use the same host rules |
| Capacity | Shared `assertNodeCapacity` (incl. reserve) on create/PATCH |
| Nodes | Only admins pick `nodeId` / `storageId` on create / clone / import (omit both for auto placement: RAM → CPU → storage); **extraMounts** admin-only (same bar as memory/disk); remote-install verifies SSH host keys (explicit trust + stored fingerprint; replace requires confirm) **and requires panel password step-up**; install script prefers ufw allow daemon port **from panel IP only** |
...
to short-lived daemon JWTs; mitigations shorten the window and remove legacy bearer.
