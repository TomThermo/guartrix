...
| Rate limits | Login / API-key / SFTP counters via `RATE_LIMIT_STORE` (`file` default under `data/rate-limits/`, or `memory`); authenticated session `/api` capped by `API_SESSION_RATE_LIMIT` (default 600/min **per userId**, IP fallback only if session has no user); panel-password step-up (`verifyAccountPassword`) capped 10/15min per user (delete server, app passwords, 2FA disable/recovery, account delete) |
