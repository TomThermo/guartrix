...
- Optional for every account under **Security** (`/account/security?tab=security`).
- Scan a QR code (or type the secret) into Google Authenticator, Authy, 1Password, … then save the one-time recovery codes.
- Login is password → 6-digit code (or a recovery code). **SFTP** accepts the panel password or an **app password** (`gtap_…`) from Account → Access.
...
- **Account → Profile**: full name, email (live availability check + re-verify on change), password change (confirm twice; authenticator code required when 2FA is on), mobile with country flag + dial code, postal address with map address-check (OpenStreetMap).
- Stored for future checkout / server sales; GDPR export includes these fields.
- Session HTTP APIs (cookie + CSRF; not `gt_` keys): `GET`/`PATCH /api/account/profile`, `GET /api/account/email-available?email=`, `GET /api/account/address-suggest?q=`, `POST /api/account/password`. Details: [Client API](client-api.md) · [API examples](api-examples.md#account).

## Export & delete account

- **Export data** (Account → Privacy) downloads a JSON copy of your profile, owned servers, recent activity, subuser memberships, API key metadata (no secrets), and billing rows.
...
- Create under **Access** → API keys. Token shown once; revoke anytime.
