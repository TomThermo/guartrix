...
| `AccountSecurityPage` | Tabs: **Profile** (contact + password), **Appearance** (theme/palettes), Security (TOTP), Access (API keys + app passwords), Notifications, Privacy (export/delete) |
...
| `account/` | Contact profile + password change, GDPR, push, Client `gt_` keys, SFTP `gtap_` passwords |
...
| Release/build | Operator-host packaging scripts (not in public tree) | Build staging, minified bundles, release and download packaging |
