...
- delete old backups
- **retention limit per server** — how many backups to keep (default **7**, max **50**); set when **creating a server**, on **Server → Backups**, or centrally in **Admin → Server backups**; older archives are pruned automatically after each new backup (manual, scheduled, or upload)
