...
- **Start** (and **Restart**) syncs `server.properties` `server-port`, host firewall rules, and Docker publish to all assigned allocations before the container boots; if the port (or properties file) was out of sync, the console shows purple `[Guartrix Daemon] NOTICE:` lines explaining the sync
