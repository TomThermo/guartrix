...
1. **Create**
   Choose name, server software family, Minecraft version (and for Paper/Purpur a **build**, Fabric/Quilt a **loader**, Forge/NeoForge a **mod loader version**), memory, disk, port, and **backup retention** (max archives kept; defaults from Admin settings). Admins can also choose the node and an optional **storage pool** (see [Install nodes — storage pools](install-nodes.md#storage-pools-admin--storage)).
