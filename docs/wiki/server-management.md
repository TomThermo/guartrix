...
Official BDS defaults to **`online-mode=false`** so the server boots without Microsoft online-services (required only for Xbox-account verification). **Allowlist/whitelist requires `online-mode=true`** — with offline mode the daemon keeps `allow-list=false` on start. Set `online-mode=true` in server properties only if the node has reliable outbound access to Microsoft. On each start the daemon also applies `enable-lan-visibility=true` and `transport=raknet` for Docker.
