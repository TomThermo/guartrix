...
npm run lint          # Biome check (formatter + lint; CI gate on full repo)
...
GitHub Actions (`.github/workflows/ci.yml`) on push/PR to `main`: install → Prisma generate → build shared → typecheck api/web/daemon/node-agent/shared → `npm run lint` → `npm run check:openapi` → `npm test` → `npm run test:coverage` → `npm audit --omit=dev --audit-level=critical` → `npm run build`.
