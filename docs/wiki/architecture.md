...
| **Disk** (`diskMb`) | Enforced on panel uploads, file writes, and SFTP. Start refused if already over. Running servers are **stopped** when over quota. Client API: **admin-only** to change |
...
**Allocations** track IP:port inventory per node:
...
While a server is **RUNNING**, the daemon opens a Docker Engine API stats stream (`/containers/…/stats?stream=1`). Samples are:
...
- **Server files:** on the node under `data/servers/<serverId>/` (or daemon `DATA_DIR`). Optional **storage pools** (Admin → Storage) place data at `{mountPoint}/servers/<id>` on a linked node (local path or NFS; one pool can link to multiple nodes). File **list/read** does not run a recursive `chown`; ownership is fixed on start and on write/upload/SFTP paths.
