...
Open the panel URL → sign in with your username and password. The top nav has **Servers**, **New server** (when allowed), a **Resources** menu (Wiki / API), **Admin** (admins), and your account menu (**Profile**, **Appearance**, **Security**, **Billing**, **Sign out**).

Admins open **Admin** for the dedicated admin area. The sidebar is grouped: **Overview** (Status, Go-live, Activity) → **Fleet** (Servers, Nodes, Users) → **Config** (Settings, Security, API keys) → **Commerce** (Billing, License). Fleet links show live **counts**. The footer shows the product version (`vX.Y.Z`).

![Login](assets/01-login.png)

### Register

Self-serve registration (when enabled). New accounts start with **0** servers / RAM / databases until an admin raises quotas or the user pays via **Billing** (Mollie). Admins can require Cloudflare Turnstile (“I'm not a robot”) on **login and registration** under **Admin → Security**.
...
Navbar account menu → **Profile** (full name, email with live availability check + re-verify on change, password change — confirm twice; authenticator code when 2FA is on — mobile with country flag + dial code, address check via map lookup), **Appearance** (theme + 12 palettes), **Security** (TOTP / recovery codes), **Access**, **Notifications**, **Privacy**, and **Billing**. When 2FA is on, sign-in asks for a code after the password. Admins can reset a user's 2FA from **Users**. Appearance stores dark / light / system as `guartrix.theme` and the palette as `guartrix.palette` (default **Crimson Graphite**). Each palette changes surfaces, accent, and UI shape in both modes.
...
Admins manage plans and recent payments under **Admin → Billing**. Application API keys live under **Admin → API keys**.
...
**Settings** — panel-wide configuration with top tabs in operator order: **General** (branding, public URL, registration, quotas, Cloudflare DNS), **Mail**, **Alerts**, **Backup defaults** (default retention + offsite hook), **Misc**, **Go-live**. Deep-link with `?tab=` (e.g. backup defaults from Admin → Servers). Overview also links **Go-live** and **Activity**.
HTTPS, proxies, 2FA roles, Turnstile, and Redis live under **Admin → Security** (Config section in the sidebar). Application API keys live under **Admin → API keys**.
...
| Group | Tabs |
|-------|------|
| Control | Console, Resources |
| Files & data | File Manager, **SFTP**, Databases, Backups |
| Network | **Ports** |
| Game | Server Properties, **Engine** (Paper/Purpur), **World Map**, **Plugin Management**, **Modpacks** (Fabric/Forge…) |
| Players | Online Players, Whitelist, Bans |
| Ops | Schedules, Subusers, **Audit log**, **Server logs** |
...
**Version & software** live in the header toolbar (**Version** / **Software** on desktop; on small screens open **Manage**). Power controls (**Start** / **Stop** / **Restart** / **Kill**) also sit in the server header so you can act from any tab. Pick any listed Minecraft version (upgrade or downgrade with confirm), or change software (Paper ↔ Fabric, etc., with addon wipe when crossing plugin/mod ecosystems). When a newer build is available, a yellow **Update available** banner still appears above the console. **Reinstall** refreshes the runtime with optional keep-world / keep-addons; a backup is always created first.
...
WebSocket console: live logs, send commands, power controls (also mirrored in the server header), address / RAM / CPU / storage / players. Below the console, **history charts** are collapsed by default — expand them when you need roughly the last hour of CPU / RAM / network from the **daemon** ring buffer (kept on the node that runs the server — not in the panel API heap). Choice is remembered in this browser.
...
Edit `server.properties`, startup / Java settings (permission-scoped), and **extra host mounts** under Start Configuration (`settings.update`) — bind allowlisted host dirs into the container (apply on next restart). The **World** category includes **world tools**: reset selected dimensions and upload a `.zip` world (server must be stopped; backup first). **Server icon** (Minecraft multiplayer list / `server-icon.png`) is edited by clicking the icon in the server header, or via **Change icon** near the top of **General**.
